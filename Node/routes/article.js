...
function formatArticle(article){
    ...
    for (var i = 0; i < article.title.length; i++) {
        ...
        article.title[i]= titleline.compact().replace(/[^0-9a-zA-Z\s!"§%&\/\(\)=\?,;\.:\-_öüäÖÜÄ']/g,"");
    }
    article.article =article.article.split(breaker);
    for (var j = 0; j < article.article.length; j++) {
        var articleline = article.article[j];
        article.article[j]= articleline.compact().replace(/[^0-9a-zA-Z\s!"§%&\/\(\)=\?,;\.:\-_öüäÖÜÄ']/g,"");
    }
    article.date= formatDate(article.date).replace(/[^0-9a-zA-Z\s!"§%&\/\(\)=\?,;\.:\-_öüäÖÜÄ']/g,"");
    return article;
}
